...
删除链表中的节点 (delete_node):

遍历链表以找到具有特定值的节点。
如果找到了该节点，则根据其位置更新前一个节点的 next 指针。
释放被删除节点的内存。
显示链表 (display):
...
使用 delete_node 函数删除值为 2 的节点。
再次使用 display 函数显示链表的内容。
程序正常退出。
*/
...
void delete_node(Node **head, int data)
{
    ...
    while (current && current->data != data) // 遍历链表直到找到目标节点或链表结束
    {
        previous = current; // 更新前一个节点
        current = current->next; // 移动到下一个节点
    }
    if (!current) // 如果没有找到目标节点
    {
        printf("Element not found.\n"); // 输出提示信息
        return; // 结束函数
    }
    if (!previous) // 如果要删除的是链表头
    {
        *head = current->next; // 更新链表头
    }
    else // 如果要删除的是链表中间或末尾的节点
    {
        previous->next = current->next; // 更新前一个节点的 next 指针
    }
